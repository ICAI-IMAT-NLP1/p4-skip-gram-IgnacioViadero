...
def subsample_words(words: List[str], vocab_to_int: Dict[str, int], threshold: float = 1e-5) -> Tuple[List[int], Dict[str, float]]:
    ...
    freqs: Dict[str, float] = {word : Counter(words)[word]/len(words) for word in words}
    print("freqs hechas")
    prob_discard = {word: 1 - torch.sqrt(torch.tensor(threshold)//torch.tensor(freqs[word])) for word in freqs.keys()}
    print("prob_discard hechas")
    train_words: List[str] = [word for word in words if freqs[word] > threshold and torch.rand(1) < prob_discard[word]]
    print("train_words hechas")
...
def get_target(words: List[str], idx: int, window_size: int = 5) -> List[str]:
    ...
    for i in range(idx - window_size, idx + window_size + 1):
        if i != idx and i >= 0 and i < len(words):
...
def get_batches(words: List[int], batch_size: int, window_size: int = 5) -> Generator[Tuple[List[int], List[int]],None,None]:
    ...
    for idx in range(0, len(words), batch_size):
        ...
        for i in range(len(batch)):
            ...
            inputs.extend([input_words] * len(target_words))
